...
// Update game state at 60 Hz (1000 ms / 60 Hz = 16 ms per update).
...
int main()
{
    ...
    while (window.state != vw::GameWindow::GameState::Exit)
    {
         ...
         * update logic, not having it update its state immediately on arrival
         * of input) is to be able to use this while loop. This "lag loop" makes
         * sure that game state updates deterministically in a given interval of
         * wall time regardless of how quickly or slowly the hardware can run
         * the game. It decouples state updates from the rendering framerate.
         * This only really matters for physics calculations or animation, where
         * not having it causes obvious glitches (like Doc Mitchell's head spin
         * when playing at higher than 60 FPS: https://youtu.be/ITOrKb5HP6s).
         */
        ...
        if (window.show_fps)
        {
            ...
            font_manager.show_fps(fps, vw::g_ui_dark);
        }
